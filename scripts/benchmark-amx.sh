...
echo "Weight prepack (int8 kernel): GGML_AMX_PREPACK=1 packs VNNI tiles once"
echo "per weight matrix (costs RAM: 1.06x Q8_0 / 2.0x Q4_0 weight size)."
echo "For end-to-end numbers on a Q4_0/Q8_0 model compare:"
echo "  llama-bench -m model.q4_0.gguf -p 512 -n 0 [-t <threads>]"
echo "with and without GGML_AMX_DISABLE=1 (and GGML_AMX_PREPACK=1) in the"
echo "environment."
