...
// ---------------------------------------------------------------------------
// Weight prepacking (INT8 path, opt-in: GGML_AMX_PREPACK=1).
//
// WHY: pack_b_tile above costs ~200 vector ops per k-block, and the int8
// kernel pays it on EVERY call - even though the weights never change
// between calls. Prepacking runs exactly the same pack_b_tile once per
// weight matrix and caches the tile-ready bytes; the kernel then just
// points tileloadd at the cache. Precision is bit-identical to the
// per-call path: same bytes in VNNI order, same fp16 scales - only the
// TIMING of the packing changes, never the values.
//
// The price is RAM. One packed slot is 576 B per (16 rows x one k-block):
// 512 B payload + the 64 B scale tail. Per weight that is 36 B per 32
// values - 1.06x the Q8_0 footprint (36 vs 34 B) and 2.0x for Q4_0 (36
// vs 18 B). Q8_0 is a fair trade (bytes-for-bytes packing, no precision
// change); Q4_0 doubles the memory, which a memory-bound decode phase
// would pay for directly - the analysis in the thesis §3.5. That is why
// the whole thing is opt-in.
//
// Concurrency: the map itself is guarded by a mutex, but the critical
// section is lookup/insert only. The packing work happens OUTSIDE the
// lock, and each thread packs only the nb-blocks of its own N-range
// (the split above hands threads disjoint nb ranges). Writes to
// different nb slots never overlap, and no thread ever reads a slot
// outside its own range - so there is nothing to race on. A per-nb flag
// makes repacking idempotent across calls.
//
// Contract (same as IQK's prepacked formats / upstream's weight cache):
// the bytes at W must be IMMUTABLE once first seen. Inference weights
// are. NOTE on the key: it is a TUPLE (pointer, type, N, KB, stride),
// not the bare pointer - std::vector happily hands a freed block back
// for the next same-size allocation, so a bare-pointer cache would mix
// up successive synthetic matrices in tests (observed: repeated shapes
// failed, fresh ones passed). Same-shape same-type address reuse with
// different DATA is still out of contract; tests clear the cache
// between cases via ggml_amx_prepack_clear().
struct amx_prepack_key_t {
    const void * w;
    int          type;
    int64_t      n, kb, stride;
    bool operator<(const amx_prepack_key_t & o) const {
        return std::tie(w, type, n, kb, stride) <
               std::tie(o.w, o.type, o.n, o.kb, o.stride);
    }
};
struct amx_prepack_entry_t {
    // slot layout: data[nb][ki][half], 576 B each; half 0/1 are the two
    // 16-column B tiles of the nb-th 32-column block. Zero-initialized:
    // slots that belong to an nv1 == 0 tail are simply never written
    // past the zero fill pack_b_tile already does.
    std::unique_ptr<char[]> data;
    std::unique_ptr<std::atomic<int>[]> packed;   // one flag per nb
};
std::mutex g_prepack_mu;
std::map<amx_prepack_key_t, amx_prepack_entry_t> g_prepack;
int g_amx_prepack = -1;   // -1 = decide from the environment once

bool amx_prepack_on() {
    if (g_amx_prepack < 0) {
        const char * e = getenv("GGML_AMX_PREPACK");
        g_amx_prepack = (e && e[0] == '1') ? 1 : 0;
    }
    return g_amx_prepack == 1;
}

// Drops all cached packs. Only call when no GEMM is in flight on other
// threads (tests call it between cases). No-op when the cache is empty.
void amx_prepack_clear() {
    std::lock_guard<std::mutex> lock(g_prepack_mu);
    g_prepack.clear();
}

// Returns the packed-slot base for this W, or nullptr when prepacking is
// off / out of memory (caller falls back to per-call packing). Packs the
// caller's own [nb0, nb1) range first.
template <typename WT>
char * amx_prepack_get(int type, const char * w, int64_t w_stride, int64_t N, int64_t KB,
                       int64_t nb0, int64_t nb1) {
    if (!amx_prepack_on()) return nullptr;
    const int64_t NB = (N + 2 * TILE_N - 1) / (2 * TILE_N);
    const amx_prepack_key_t key{w, type, N, KB, w_stride};
    amx_prepack_entry_t * ent;
    {
        std::lock_guard<std::mutex> lock(g_prepack_mu);
        ent = &g_prepack[key];
        if (!ent->data) {
            ent->data.reset(new (std::nothrow) char[(size_t)NB * KB * 2 * 576]());
            ent->packed.reset(new (std::nothrow) std::atomic<int>[(size_t)NB]());
            if (!ent->data || !ent->packed) {   // OOM: quietly stay per-call
                ent->data.reset();
                ent->packed.reset();
                g_prepack.erase(key);
                return nullptr;
    ...
    for (int64_t nb = nb0; nb < nb1; ++nb) {
        if (ent->packed[nb].load(std::memory_order_acquire)) continue;
        ...
        char * base = ent->data.get() + nb * KB * 2 * 576;
        for (int64_t ki = 0; ki < KB; ++ki) {
            // byte-identical to what the kernel's per-call path produces
            // (including the zero-fill semantics of nv == 0)
            pack_b_tile<WT>((int8_t *)(base + ki * 2 * 576),
                            w + n0 * w_stride, w_stride, ki, nv0);
            pack_b_tile<WT>((int8_t *)(base + ki * 2 * 576 + 576),
                            w + (n0 + TILE_N) * w_stride, w_stride, ki, nv1);
        }
        ent->packed[nb].store(1, std::memory_order_release);
    }
    return ent->data.get();
}
...
void amx_gemm_impl(int64_t M, int64_t N, int64_t KB,
    ...
    // Weight prepacking (opt-in): pack the VNNI tiles of OUR nb-range once
    // and point tileloadd straight at the cache. nullptr = off/OOM, and
    // everything below falls back to per-call packing.
    char * pre = amx_prepack_get<WT>(
        std::is_same<WT, block_q4_0>::value ? GGML_TYPE_Q4_0 : GGML_TYPE_Q8_0,
        w, w_stride, N, KB, nb0, nb1);
    ...
    for (int64_t nb = nb0; nb < nb1; ++nb) {
        ...
        for (int64_t ki = 0; ki < KB; ++ki) {
            ...
            if (pre) {
                ...
                bt0 = (const char *)buf.b_tiles[0];
                bt1 = (const char *)buf.b_tiles[1];
            }
            ...
            AMX_TILE_LOADD(amx_tok, TMM0, bt0, TILE_N * VNNI_BLK);
            AMX_TILE_LOADD(amx_tok, TMM1, bt1, TILE_N * VNNI_BLK);
            const __m512 vdw0 = _mm512_cvtph_ps(_mm256_loadu_si256((const __m256i *)(bt0 + 512)));
            const __m512 vdw1 = _mm512_cvtph_ps(_mm256_loadu_si256((const __m256i *)(bt1 + 512)));
...
// opt-in weight prepacking for the int8 kernel (see amx_prepack_get):
// 1 = pack VNNI tiles once per weight matrix (costs RAM: 1.06x Q8_0,
// 2.0x Q4_0), 0 = pack per call (default). Weights must be immutable.
extern "C" void ggml_amx_set_prepack(int enable) {
#if GGML_AMX_AVAILABLE_COMPILER
    g_amx_prepack = enable ? 1 : 0;
#else
    (void)enable;
#endif
}

// drops all cached prepacked weights; call only with no GEMM in flight
// (used by tests, which reuse allocator addresses between cases)
extern "C" void ggml_amx_prepack_clear(void) {
#if GGML_AMX_AVAILABLE_COMPILER
    amx_prepack_clear();
#endif
}
