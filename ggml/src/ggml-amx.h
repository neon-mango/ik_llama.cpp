...
// opt-in weight prepacking for the INT8 kernel (env GGML_AMX_PREPACK=1
// does the same): VNNI tiles are packed once per weight matrix instead
// of on every call. Bit-identical results; costs extra RAM (1.06x the
// Q8_0 weight size, 2.0x Q4_0). The weight bytes must be immutable.
GGML_API void ggml_amx_set_prepack(int enable);

// drop all cached prepacked weights (call only with no GEMM in flight;
// tests use it because allocator address reuse would alias cache keys)
GGML_API void ggml_amx_prepack_clear(void);
