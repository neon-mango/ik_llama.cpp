...
bool check_one(int type, int64_t M, int64_t N, int64_t K, int nth, std::mt19937 & rng, bool bf16, double tol) {
    ...
    // allocator address reuse would alias prepack cache keys between
    // cases; clearing is a cheap no-op when prepacking is off/empty
    ggml_amx_prepack_clear();
...
int run_correctness() {
    ...
    // The bf16 section is skipped on machines where tdpbf16ps does not
    // execute despite the CPUID flag (KVM guests, see quirks.md §11).
    const bool has_bf16 = ggml_amx_has_bf16();
    if (!has_bf16) printf("BF16 kernel not executable on this machine - skipping BF16 section\n");
    for (int mode = 0; mode < 2; ++mode) {
        const bool bf16 = (mode == 0);
        if (bf16 && !has_bf16) continue;
    ...
    // prepack: the int8 kernel reading pre-cached VNNI tiles must match
    // the same reference as the per-call packing (bit-identical path)
    {
        printf("== correctness: INT8 kernel + weight prepack (GGML_AMX_PREPACK) ==\n");
        ggml_amx_set_prepack(1);
        for (const auto & s : shapes) {
            rc |= check_one<block_q4_0>(GGML_TYPE_Q4_0, s.m, s.n, s.k, 1, rng, false, 1e-4);
            rc |= check_one<block_q8_0>(GGML_TYPE_Q8_0, s.m, s.n, s.k, 1, rng, false, 1e-4);
        }
        const shape_t pp_mt[] = {{31, 47, 96}, {512, 512, 512}};
        for (const auto & s : pp_mt) {
            rc |= check_one<block_q4_0>(GGML_TYPE_Q4_0, s.m, s.n, s.k, 4, rng, false, 1e-4);
            rc |= check_one<block_q8_0>(GGML_TYPE_Q8_0, s.m, s.n, s.k, 4, rng, false, 1e-4);
        }
        ggml_amx_set_prepack(0);
    }
...
int run_bench() {
    ...
    const bool has_bf16 = ggml_amx_has_bf16();
    if (!has_bf16) printf("BF16 kernel not executable on this machine - benching INT8 only\n");
    for (int mode = 0; mode < 2; ++mode) {
        const bool bf16 = (mode == 0);
        if (bf16 && !has_bf16) continue;
        ...
        if (!bf16) {
            // steady-state prepack numbers: bench_one's warmup call fills
            // the cache, the timed reps reuse it
            printf("-- int8 kernel + weight prepack --\n");
            ggml_amx_set_prepack(1);
            bench_one(GGML_TYPE_Q4_0, 512, 4096, 4096, nth, rng, false);
            bench_one(GGML_TYPE_Q8_0, 512, 4096, 4096, nth, rng, false);
            bench_one(GGML_TYPE_Q4_0, 512, 4096, 11008, nth, rng, false);
            bench_one(GGML_TYPE_Q4_0, 2048, 4096, 4096, nth, rng, false);
            ggml_amx_set_prepack(0);
        }
    }
    return 0;
